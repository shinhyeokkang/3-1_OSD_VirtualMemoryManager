...
public class PageTable{
    ...
    public int get(int p_num){
        int frameNumber = this.table[p_num].getFrameNumber();//p_num 에 해당하는 위치의 frame_number를 출력
        if(frameNumber == -1){
    ...
    public void add(int p_num, int f_num){
        this.table[p_num] = new PageTableItem(f_num, true);// p_num 에 해당하는 위치에 f_num 입력
    }
}
